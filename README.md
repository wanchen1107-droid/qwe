...
<script>
    ...
    function updateObstacles() {
        ...
        if (++obstacleTimer > Math.max(40, 100 - (gameSpeed * 1.5))) {
            createObstacle();
            obstacleTimer = 0;
        }
    }
    ...
    function createObstacle() {
        ...
        else if (rand > 0.6) config = { type: 'cactus', width: 40, height: 60, y: 0 };
    ...
    function updateClouds() {
        if (++cloudTimer > 100) {
            ...
            container.appendChild(el);
            setTimeout(() => { if(el.parentNode) container.removeChild(el); }, 30000);
            cloudTimer = 0;
        }
    }
